...
class MainWindow(QMainWindow, Ui_ECGB_Window):
    ...
    def config(self):
        ...
        self.waveform_timer.timeout.connect(self.update_simulated_waveform)
            ...
            if self.mPackUnpck.unpackData(byte):
                ...
                if unpacked[0] == 0x01:  # 确认模块ID
                    if unpacked[2] == 0xA1:  # 确认二级ID为心率
                        ...
                        # 保持数据队列长度
                        max_points = 5 * 20  # 存储5秒数据（假设250Hz采样率）
    ...
    def update_simulated_waveform(self):
